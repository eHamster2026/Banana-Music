...
- 自动化助手默认在本地 `dev` 分支作业；开始修改前先确认当前分支，必要时切换到 `dev`。
- 每次代码或文档修改完成并通过必要验证后，助手应将本次相关改动提交到 `dev` 分支。
- 提交时只包含本次任务相关文件；不得混入用户已有的未提交改动或无关改动。
- 只有在用户显式要求“提交到 main”“合并到 main”“推送”等操作时，才允许把 `dev` 的成果同步到 `main` 并推送远端 GitHub。
- 推送到远程 GitHub 完成后，需要继续发布 Docker 镜像到 Docker Hub；发布前确认镜像名、标签和 Docker Hub 凭据可用。
- 未收到显式要求前，不得推送远端，也不得改写远端分支。
...
- 本地文件上传逻辑在 `src/localUpload.js`：`uploadSingleFile` → `pollUploadJob` → `createTrack`（只传 `file_key`，元数据由后端解析）
...
| 工具 | 端点 | 权限 | 用途 |
|---|---|---|---|
| `search_tracks` | `GET /rest/search3?query={query}` | 可匿名 | 搜索本地曲目、专辑、艺术家；已登录时可合并 `plugin_hits` |
| `list_tracks` | `GET /rest/x-banana/admin/tracks?skip={skip}&limit={limit}&q={query}&missing_metadata={bool}` | 管理员 | 分页列出全库曲目，支持元数据缺失过滤 |
| `get_library_stats` | `GET /rest/x-banana/admin/stats` | 管理员 | 查看曲库统计和元数据缺失数量 |
| `update_metadata` | `PUT /rest/x-banana/admin/tracks/{track_id}` | 管理员 | 更新单曲标题、艺人、专辑、音轨号、时长 |
| `batch_update_metadata` | `POST /rest/x-banana/admin/tracks/batch-update` | 管理员 | 批量更新元数据，单次最多 50 条 |
| `upload_file` | `POST /rest/x-banana/tracks/upload-file` | 认证 | 上传音频文件，返回后台任务 `job_id` |
| `upload_status` | `GET /rest/x-banana/tracks/upload-status/{job_id}` | 认证 | 轮询上传任务状态 |
| `create_track` | `POST /rest/x-banana/tracks/create` | 认证 | 用 `file_key` 将已上传文件正式写入曲库 |
...
- 批量修复“未知艺人”：先 `GET /rest/x-banana/admin/stats` 查看缺失数量，再用 `GET /rest/x-banana/admin/tracks?missing_metadata=true&limit=200` 翻页收集曲目，最后按每批不超过 50 条调用 `POST /rest/x-banana/admin/tracks/batch-update`
- 整理专辑：先 `GET /rest/search3?query=<专辑关键词>` 找到相关曲目，再批量设置 `album_title` 和 `track_number`
...
批量接口的部分失败不会触发 HTTP 错误码，而是通过 `failed` 字段返回。上传流程中 `file_key` 与服务端暂存的 `audio_hash` 必须对应同一个文件，不可混用。
...
| 表 | 用途 |
|---|---|
| `tracks` | 曲目（含 audio_hash、audio_fingerprint）|
