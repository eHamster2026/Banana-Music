...
```bash
cd backend && UV_PROJECT_ENVIRONMENT=venv uv sync        # 安装后端依赖到 backend/venv
...
# 生产环境建议至少修改：
# - SECRET_KEY
# - DATABASE_URL（如需使用 PostgreSQL）
# - CORS_ORIGINS（限制为实际域名）

# 3. 启动（两个终端）
bash scripts/dev-local.sh backend    # 后端 :8000，自动重载
bash scripts/dev-local.sh frontend   # 前端 :5173，热更新，API/资源同源代理到后端
```
...
- 开发环境可使用 `DEMO_USERNAME`/`DEMO_PASSWORD` 登录（默认 `demo` / `demo123`）。
- Vite 开发服务器默认将 `/rest`、`/resource`、`/covers` 代理到 `BANANA_DEV_PROXY_TARGET`（默认 `http://localhost:8000`）；如后端在其他地址，启动前显式设置该变量。

### 上传与元数据处理

前端先解析本地文件标签、封面和音频 MD5/哈希；上传前通过 `GET /rest/x-banana/tracks/exists-by-hash` 查重。新文件调用 `POST /rest/x-banana/tracks/upload-file` 落盘并返回 `job_id`，前端轮询 `GET /rest/x-banana/tracks/upload-status/{job_id}` 至 `state: done` 后获得 `file_key`。封面先通过 `POST /rest/x-banana/tracks/covers/upload` 上传得到图片 id；如需 LLM 清洗，前端显式调用 `POST /rest/x-banana/plugins/llm-metadata/parse-metadata`。最后 `POST /rest/x-banana/tracks/create` 提交 `file_key`、客户端解析/清洗后的元数据和可选 `cover_id`。

API 当前统一挂载在 `/rest/*`：音乐库接口采用 Subsonic 风格命名（JSON-only），项目专属能力位于 `/rest/x-banana/*`。详见 [`docs/api.md`](docs/api.md)。
...
| `BANANA_DEV_PROXY_TARGET` | `http://localhost:8000` | Vite 开发服务器代理目标；仅用于本地开发 |
...
大量导入时，LLM 清洗应在客户端/脚本侧显式完成，服务端不再维护等待清洗的上传队列。推荐工作流：

1. 用 `bulk_import.py process` 离线预处理所有文件（转码 + LLM 清洗 + 标签写入副本）
2. 将处理好的文件通过 `bulk_import.py upload --no-parse-metadata` 或 `process --upload --no-parse-metadata` 批量上传到服务
3. 脚本按客户端上传流程提交 metadata / cover_id；服务端只负责暂存文件、查重和入库
...
# 后端（从仓库根目录）
cd backend
UV_PROJECT_ENVIRONMENT=venv uv sync --extra dev             # 安装后端测试依赖到 backend/venv
...
| [`docs/bulk-import.md`](docs/bulk-import.md) | 批量导入指南：预处理脚本详解、推荐工作流、如何通过参数控制服务端清洗 |
