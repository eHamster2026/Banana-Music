...
`POST /rest/x-banana/tracks/upload-file` 先落盘并返回 `job_id`，前端轮询 `GET /rest/x-banana/tracks/upload-status/{job_id}` 至 `state: done` 后获得 `file_key`；随后 `POST /rest/x-banana/tracks/create` 请求体**仅需** `{ "file_key" }`，元数据由服务端自动解析，`parse_upload` 流水线在后台异步执行（重启不丢任务）。如需手动修改元数据，使用管理端 `PUT /rest/x-banana/admin/tracks/{id}` 或批量接口。
...
3. 服务端按文件标签直接入库，不为这些请求入队 `parse_upload`
