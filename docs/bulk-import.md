...
本文档介绍如何使用 `scripts/bulk_import.py` 在导入前完成格式转换与元数据清洗，以及大量导入时为何应关闭服务端的 LLM 清洗逻辑。
...
1. [预处理脚本概览](#1-预处理脚本概览)
2. [脚本详解](#2-脚本详解)
3. [为什么要在服务端关闭 LLM 清洗](#3-为什么要在服务端关闭-llm-清洗)
4. [批量导入推荐工作流](#4-批量导入推荐工作流)
5. [如何关闭服务端 LLM 清洗](#5-如何关闭服务端-llm-清洗)
...
```
SHA-256 → GET /tracks/check-hash → POST /tracks/upload-file
→ GET /tracks/upload-status/{job_id} 轮询 → POST /tracks/create
```

默认上传写库后会让服务端继续执行 `parse_upload` 元数据清洗；如只想使用脚本已写入的标签，可加 `--no-parse-metadata`，脚本会在 `POST /tracks/create` 中传 `parse_metadata: false`。
...
Banana Music 服务端在每首曲目上传后会将 LLM 清洗任务写入 `parse_upload_tasks` 表，由 `parse_upload_worker` 异步消费。这在日常少量上传时运转良好，但**批量导入时会产生以下问题**：
...
processed/ 目录            ← 标签整洁、格式统一的 FLAC / MP3
      │
      ▼
scripts/bulk_import.py upload 或 process --upload
                             ← 此时关闭服务端 LLM 清洗
      │
      ▼
重新启用服务端 LLM 清洗     ← 恢复日常上传的自动处理
```
...
2. **关闭服务端 LLM 清洗**：见 [第 5 节](#5-如何关闭服务端-llm-清洗)

3. **批量上传**：将 `processed/` 中的文件上传至服务
...
4. **重新启用 LLM 清洗**：恢复插件或配置

---

## 5. 如何关闭服务端 LLM 清洗

服务端 LLM 清洗由 `llm-metadata` 插件提供。关闭方式有两种，按偏好选择其一。

### 方式一：通过管理员面板禁用插件（推荐）

在 Banana Music Web 界面进入「管理员 → 插件」，找到 **LLM Metadata Parser**，将其切换为「禁用」。

效果：`parse_upload_worker` 仍然运行，但该阶段无注册回调，任务会被立即跳过并清除，不产生任何 Ollama 请求。导入完成后重新启用即可。

### 方式二：直接编辑插件状态文件

```bash
# 禁用
echo '{"enabled": false}' > plugins/llm-metadata/state.json

# 导入完成后重新启用
echo '{"enabled": true}' > plugins/llm-metadata/state.json
```

修改后需重启服务端使插件状态生效。

> **注意**：不建议使用 `BANANA_TESTING=true` 来跳过 LLM 清洗——该环境变量同时会禁用指纹计算、数据库初始化等功能，仅适用于测试环境。
